...
def output_json(content):
    return output(simplejson.dumps(content, default=str))


def output_success(content):
    return output_json({
        "errno": 0,
        "data": content
    })
...
def output_error(errno, msg):
    return output(simplejson.dumps({
        "errno": errno,
        "errmsg": msg
    }, default=str))
...
def get_dict_md5(data: dict) -> str:
    m_str = ""
