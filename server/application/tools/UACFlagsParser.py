...
class UACFlagsParser(object):
    ...
    def has_one_flag(self, flags, check_flag) -> bool:
        result = self.parse(flags)
        return result[check_flag]

    def parse_security(self, flag) -> dict:
        """
            只返回与安全相关的标志
        """
        result = {}
        security_list = ["ACCOUNT_DISABLE", "PASSWD_NOTREQD", "ENCRYPTED_TEXT_PWD_ALLOWED", "DONT_EXPIRE_PASSWORD",
                         "SMARTCARD_REQUIRED", "TRUSTED_FOR_DELEGATION", "NOT_DELEGATED", "USE_DES_KEY_ONLY",
                         "DONT_REQ_PREAUTH", "PASSWORD_EXPIRED"]
        temp_result = self.parse(flag)
        for f in security_list:
            if f in temp_result:
                result[f] = temp_result[f]
        return result
...
if __name__ == '__main__':
    print(UACFlagsParser().parse(4096))
