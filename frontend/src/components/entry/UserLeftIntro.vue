<template>
    <div class="user-left-intro">
        <div class="avatar-info">
            <div class="avatar-outer">
                <div class="avatar-wrapper">
                    <Icon color="#0294FF" class="avatar-icon" size="35" custom="iconfont icon-user" />
                </div>
                <div>
                    <Icon v-if="sensitive" title="敏感账户" color="#19be6b" class="avatar-flag" size="30" custom="iconfont icon-privilege-o" />
                </div>
                <div>
                    <Icon v-if="disabled" title="禁用账户" color="#FB2C2C" class="avatar-flag" size="30" type="ios-remove-circle" />
                </div>
            </div>
            <div class="name">
                {{displayName}}
            ...
            <div class="desc">
                ...
                <div class="detail" v-if="info['department'] == ''">{{info["description"]}}</div>
            </div>
        </div>
        <div class="detail-info">
            <div class="base-info">
                <div>
                    <div class="item-name">员工编号</div>
                    <div class="item-value">{{info["employeeNumber"]}}</div>
                </div>
                <div>
                    <div class="item-name">域账号</div>
                    <div class="item-value">{{info["sAMAccountName"]}}</div>
                </div>
                <div>
                    <div class="item-name">所属域</div>
                    <div class="item-value">{{domain}}</div>
                </div>
                <div>
                    <div class="item-name">邮箱</div>
                    <div class="item-value">{{info["mail"]}}</div>
                </div>
                <div>
                    <div class="item-name">上级</div>
                    <div class="item-value">{{info["manager"]}}</div>
                </div>
            </div>
