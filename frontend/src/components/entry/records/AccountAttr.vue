    ...
    computed: {
        ...
        description() {
            ...
            return function (createElement) {
                ...
                result_list.push(" 修改了账户UAC，")
                if (that.add_uac_list.length > 0) {
                    result_list.push("增加了 ")
                    result_list.push(that.add_uac_list.join("|"))
                    result_list.push(" 属性")
                }

                if (that.add_uac_list.length > 0 && that.remove_uac_list.length > 0) {
                    result_list.push(", ")
                }

                if (that.remove_uac_list.length > 0) {
                    result_list.push("减少了 ")
                    result_list.push(that.remove_uac_list.join("|"))
                    result_list.push(" 属性")
                }
        ...
        add_uac_list() {
            return this.record["data"]["uac_change"][0]
        },
        remove_uac_list() {
            return this.record["data"]["uac_change"][1]
        }
    },
    methods: {
